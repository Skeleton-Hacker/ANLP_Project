...
if is_main:
    # Only the main process downloads/extracts resources
    resources = [
...
# Sync all processes so others wait until download is done
...
class SemanticSentenceChunker:
    """Simple sentence splitter for preprocessing with stopword removal and lemmatization."""

    def __init__(self):
        """Initialize the sentence chunker with stopword removal and lemmatization enabled."""
        self.stop_words = set(stopwords.words('english'))
        self.lemmatizer = WordNetLemmatizer()

    def _clean_and_process_sentence(self, sentence: str) -> str:
        """Clean sentence by removing stopwords and applying lemmatization.

        Args:
            sentence: The sentence to process.

        Returns:
            Processed sentence with stopwords removed and words lemmatized.
        """
    ...
    def split_into_sentences(self, text: str) -> List[str]:
        """Split text into sentences with NLP preprocessing (stopword removal and lemmatization).

        Args:
            text: The text to split into sentences.

        Returns:
            List of cleaned sentences with stopwords removed and words lemmatized.
        """
        ...
        # Process sentences with stopword removal and lemmatization
        processed_sentences = []
        for sentence in sentences:
            sentence = sentence.strip()
            if sentence:
                sentence = self._clean_and_process_sentence(sentence)
